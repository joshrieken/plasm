defmodule Plasm.LaterThanTest do
  ...
  test ".later_than with an atom field name and DateTime" do
    # Arrange
    ...
    # Act
    user_ids = User |> Plasm.later_than(:inserted_at, date_time) |> Repo.all() |> Enum.map(&(&1.id))
  ...
  test ".later_than with an atom field name and castable date time string" do
    # Arrange
    castable_string = "2016-07-27T00:00:00Z"
    earlier_user = insert(:user, inserted_at: "2016-07-26T00:00:00Z")
    exact_match_user = insert(:user, inserted_at: castable_string)
    ...
    # Act
    user_ids = User |> Plasm.later_than(:inserted_at, castable_string) |> Repo.all() |> Enum.map(&(&1.id))
  ...
  test ".later_than with an atom field name and Date" do
    # Arrange
    ...
    # Act
    user_ids = User |> Plasm.later_than(:inserted_at, date) |> Repo.all() |> Enum.map(&(&1.id))
  ...
  test ".later_than with an atom field name and castable date string" do
    # Arrange
    castable_string = "2016-07-27"
    earlier_user = insert(:user, inserted_at: "2016-07-26T00:00:00Z")
    exact_match_user = insert(:user, inserted_at: castable_string <> "T00:00:00Z")
