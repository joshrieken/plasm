defmodule Plasm.AtOrLaterThanTest do
  use Plasm.Case
  ...
  test ".at_or_later_than with a DateTime" do
    # Arrange
    ...
    # Act
    user_ids = User |> Plasm.at_or_later_than(:inserted_at, date_time) |> Repo.all() |> Enum.map(&(&1.id))
  ...
  test ".at_or_later_than with a castable string" do
    # Arrange
    castable_string = "2016-07-27T00:00:00Z"
    earlier_user = insert(:user, inserted_at: "2016-07-26T00:00:00Z")
    exact_match_user = insert(:user, inserted_at: castable_string)
    ...
    # Act
    user_ids = User |> Plasm.at_or_later_than(:inserted_at, castable_string) |> Repo.all() |> Enum.map(&(&1.id))
